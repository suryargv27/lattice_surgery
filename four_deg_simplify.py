...
def simplify_graph(g):
    ...
    # Split vertices with degree > 4
    while True:
        # Find first vertex with degree > 4
        v = None
        for candidate in g.vertices():
            if (
                g.type(candidate) != zx.VertexType.BOUNDARY
                and g.vertex_degree(candidate) > 4
            ):
                v = candidate
                break
        ...
        # Calculate split sizes (sequential distribution)
        m = len(neighbours)
